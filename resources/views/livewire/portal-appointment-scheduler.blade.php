<section class="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
    <aside class="space-y-5">
        <div class="rounded-3xl bg-violet-600 p-6 text-white shadow-2xl shadow-violet-900/20">
            ...
            <p class="mt-3 text-sm font-medium leading-6 text-violet-100">
                Elige medico, servicio y fecha para ver horarios realmente libres sin recargar la pagina.
            </p>
        </div>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60 sm:p-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div wire:loading wire:target="medicoId,servicioId,fecha" class="rounded-full bg-violet-50 px-4 py-2 text-xs font-bold text-violet-700">
                    ...
                    <div>
                        ...
                        <div class="mt-3 grid gap-2 sm:grid-cols-3">
                            @foreach($slots as $slot)
                                <label wire:key="slot-{{ $slot['value'] }}" class="flex cursor-pointer items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 transition hover:border-violet-200 hover:bg-violet-50 has-[:checked]:border-violet-600 has-[:checked]:bg-violet-600 has-[:checked]:text-white">
