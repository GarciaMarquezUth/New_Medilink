<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-extrabold tracking-tight text-slate-950 sm:text-3xl">Registrar médico</h1>
        </div>
    </x-slot>
    ...
    <form action="{{ route('medicos.store') }}" method="POST" enctype="multipart/form-data" class="mx-auto max-w-4xl space-y-6">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60 sm:p-8">
            <div class="mb-6">
                <h2 class="text-lg font-extrabold text-slate-950">Información profesional</h2>
                <p class="mt-1 text-sm font-medium text-slate-500">Completa los datos del médico y vincula su usuario si ya existe.</p>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <x-text-input id="telefono" name="telefono" value="{{ old('telefono') }}" class="mt-2" />
                    <x-input-error :messages="$errors->get('telefono')" />
                </div>

                <div class="sm:col-span-2">
                    <x-input-label for="especialidad" value="Especialidad" />
                    <x-text-input id="especialidad" name="especialidad" value="{{ old('especialidad') }}" class="mt-2" required />
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-violet-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-violet-600/20 transition hover:-translate-y-0.5 hover:bg-violet-700">Guardar médico</button>
        </div>
    </form>
</x-app-layout>
