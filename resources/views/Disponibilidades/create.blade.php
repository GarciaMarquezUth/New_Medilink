<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-extrabold tracking-tight text-slate-950 sm:text-3xl">Registrar disponibilidad</h1>
        </div>
    </x-slot>

    <form action="{{ route('disponibilidades.store') }}" method="POST" class="mx-auto max-w-4xl space-y-6">
        @csrf
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60 sm:p-8">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm font-medium text-slate-500">Define bloques semanales para que el sistema valide las citas.</p>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <select id="medico_id" name="medico_id" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/10" required>
                        ...
                        @foreach($medicos as $medico)
                            <option value="{{ $medico->id }}" {{ (int) old('medico_id') === $medico->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="dia_semana" name="dia_semana" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/10" required>
                        ...
                        @foreach($diasSemana as $dia => $nombre)
                            <option value="{{ $dia }}" {{ (int) old('dia_semana') === $dia ? 'selected' : '' }}>{{ $nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="hora_inicio" type="time" name="hora_inicio" value="{{ old('hora_inicio') }}" class="mt-2" required />
                    <x-input-error :messages="$errors->get('hora_inicio')" />
                </div>

                <div>
                    <x-input-label for="hora_fin" value="Hora fin" />
                    <x-text-input id="hora_fin" type="time" name="hora_fin" value="{{ old('hora_fin') }}" class="mt-2" required />
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-violet-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-violet-600/20 transition hover:-translate-y-0.5 hover:bg-violet-700">Guardar disponibilidad</button>
        </div>
    </form>
</x-app-layout>
