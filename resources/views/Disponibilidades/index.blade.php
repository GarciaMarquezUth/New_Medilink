                ...
                <table class="min-w-full divide-y divide-slate-100">
                    <thead class="bg-slate-50/80">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-extrabold uppercase tracking-wider text-slate-500">Día</th>
                            <th class="px-6 py-4 text-left text-xs font-extrabold uppercase tracking-wider text-slate-500">Horario</th>
                        ...
                        @forelse($disponibilidades as $disponibilidad)
                            <tr class="transition hover:bg-violet-50/40">
                                <td class="whitespace-nowrap px-6 py-4">
                                    <p class="font-bold text-slate-950">{{ $disponibilidad->medico->nombre }} {{ $disponibilidad->medico->apellido }}</p>
                                    <p class="text-sm font-medium text-slate-500">{{ $disponibilidad->medico->especialidad }}</p>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-semibold text-slate-700">{{ $disponibilidad->diaNombre() }}</td>
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-semibold text-slate-700">
                                    {{ substr($disponibilidad->hora_inicio, 0, 5) }} - {{ substr($disponibilidad->hora_fin, 0, 5) }}
                                </td>
                                <td class="whitespace-nowrap px-6 py-4">
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-extrabold ring-1 {{ $disponibilidad->activo ? 'bg-emerald-50 text-emerald-700 ring-emerald-100' : 'bg-slate-100 text-slate-600 ring-slate-200' }}">
                                        {{ $disponibilidad->activo ? 'Activo' : 'Inactivo' }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-6 py-4 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        @can('disponibilidades.editar')
                                            <a href="{{ route('disponibilidades.edit', $disponibilidad->id) }}" class="rounded-xl bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700 transition hover:bg-violet-100">Editar</a>
                                        @endcan
                                        @can('disponibilidades.eliminar')
                                            <form action="{{ route('disponibilidades.destroy', $disponibilidad->id) }}" method="POST" onsubmit="return confirm('¿Eliminar esta disponibilidad?');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <p class="text-sm font-semibold text-slate-500">Aún no hay disponibilidades registradas.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
