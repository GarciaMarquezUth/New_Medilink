@php
    $user = Auth::user();
    $estadoLabels = \App\Models\Cita::estados();
    $estadosOcupantes = \App\Models\Cita::estadosOcupantes();
    $isPacienteDashboard = $user && $user->hasRole('paciente') && ! $user->hasAnyRole(['admin', 'recepcionista', 'medico']);
    $isMedicoDashboard = $user && $user->hasRole('medico') && ! $user->hasAnyRole(['admin', 'recepcionista']);
    ...
    if ($isPacienteDashboard) {
        ...
        $historialCitas = $citasPaciente
            ->reject(fn ($cita) => $cita->fecha_hora->isFuture() && in_array($cita->estado, $estadosOcupantes, true))
        ...
        $citasHoy = $citasMedico
            ->filter(fn ($cita) => $cita->fecha_hora->isToday())
            ->values();
        ...
        $citasPendientesMedico = $citasMedico
            ->filter(fn ($cita) => in_array($cita->estado, $estadosOcupantes, true))
            ->values();

        $citasAtendidasMedico = $citasMedico->where('estado', \App\Models\Cita::ESTADO_ATENDIDA)->count();
        ...
        $citasPendientes = \App\Models\Cita::whereIn('estado', $estadosOcupantes)->count();
    }
@endphp
