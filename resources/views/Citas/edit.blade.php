    ...
    <form action="{{ route('citas.update', $cita->id) }}" method="POST" class="mx-auto max-w-4xl space-y-6">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60 sm:p-8">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="medico_id" name="medico_id" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/10" required>
                        ...
                        @foreach($medicos as $m)
                            <option value="{{ $m->id }}" {{ (int) $selectedMedicoId === $m->id ? 'selected' : '' }}>
                                {{ $m->nombre }} {{ $m->apellido }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="fecha_hora" type="datetime-local" name="fecha_hora" value="{{ old('fecha_hora', $cita->fecha_hora ? \Illuminate\Support\Carbon::parse($cita->fecha_hora)->format('Y-m-d\TH:i') : '') }}" class="mt-2" required :disabled="! $selectedMedicoId || ! $selectedServicioId" />
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/60 sm:p-8">
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm font-medium text-slate-500">Actualiza el estado financiero, monto y método de pago de la cita.</p>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-3">
                <div>
                    ...
                    <select id="estado_pago" name="estado_pago" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/10">
                        @foreach($estadosPago as $estadoPago => $label)
                            <option value="{{ $estadoPago }}" @selected(old('estado_pago', $cita->estado_pago ?: \App\Models\Cita::PAGO_PENDIENTE) === $estadoPago)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="monto_pagado" type="number" name="monto_pagado" value="{{ old('monto_pagado', $cita->monto_pagado) }}" class="mt-2" min="0" step="0.01" placeholder="0.00" />
                ...
                <div>
                    ...
                    <select id="metodo_pago" name="metodo_pago" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/10">
                        ...
                        @foreach($metodosPago as $metodoPago => $label)
                            <option value="{{ $metodoPago }}" @selected(old('metodo_pago', $cita->metodo_pago) === $metodoPago)>{{ $label }}</option>
                        @endforeach
                    </select>
